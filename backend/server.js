    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (!data) {
        return res.status(404).json({
          error: "Unable to find note with given ID belonging to this user.",
        });
      }
      ...
      const responseValue = "Document with ID " + noteId + " properly deleted";
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    const validParams = ["title", "content"];

    for (const param in updatedNote) {
